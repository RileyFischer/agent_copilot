...
listing_prompt ="Write a professional description for listing homes for sale using adjectives to praise the property as seen on zillow or redfin"
...
with col2:
	...
	move_in_flag = st.radio('Ready to move in?', ['Yes','No'])
	luck_val = st.radio('Feeling lucky?', temp_list_vals)

	if move_in_flag == 'Yes':
		move_val = 'Ready to move in.'
	else:
		move_val = ''


if st.button('Generate Listing Description! 🧑‍🚀'):
	property_prompt = "Property: " +str(prop_val[0])+" in "+str(city_val)+" "+str(stat_val)+", "+str(int(num_beds))+" bedrooms, "+str(int(num_bath))+" bathrooms, "+str(int(num_sqft))+" sqft, "+str(int(num_park))+" parking spot(s), built in "+str(int(blt_year))+", "+str(prop_det)+", "+str(intr_det)+", "+str(kchn_det)+", "+str(otdr_det)+", "+str(addn_det)+","+str(move_val)+" Use appropriate adjectives to make the property desirable."
